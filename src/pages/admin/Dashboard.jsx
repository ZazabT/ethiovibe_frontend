      ...
      hover:scale-105 hover:shadow-lg border ${stat.borderColor}`}
          ...
          backdrop-blur-sm px-2 py-1 rounded-lg text-xs font-medium">
            ...
            <div className="text-2xl font-bold text-gray-800">
              {(orderLoading || productLoading || userLoading) ? (
                <ImSpinner2 className="animate-spin inline-block h-8 w-8 text-gray-500" />
              ) : (
                stat.value
              )}
            </div>
        ...
        <div className="overflow-x-auto">
          {orderLoading ? (
            <div className="flex justify-center items-center py-8">
              <ImSpinner2 className="animate-spin text-pink-500 text-3xl" />
            </div>
          ) : (
                  ...
                  <tr key={order._id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      ${order.deliveryStatus === 'DELIVERED'
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${order.deliveryStatus === 'DELIVERED'
                          ? 'bg-green-500'
                          : order.deliveryStatus === 'PENDING'
                            ? 'bg-yellow-500'
                            : 'bg-gray-500'
              ...
              products?.filter(p => p.countInStock < 30).slice(0, 5).map((product) => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className="relative">
                      ...
                      <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-white border border-gray-100 flex items-center justify-center">
                        <div className={`w-2 h-2 rounded-full ${product.countInStock < 10 ? 'bg-red-500' : 'bg-yellow-500'
                          }`} />
                      </div>
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${product.countInStock < 10
